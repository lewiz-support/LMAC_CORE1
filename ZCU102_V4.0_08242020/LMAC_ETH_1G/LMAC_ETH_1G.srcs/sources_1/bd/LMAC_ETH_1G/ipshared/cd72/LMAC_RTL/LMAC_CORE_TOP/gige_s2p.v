...
module gige_s2p (
		...
		ctrl_input,				 //i-1, Ctrl in signal from PHY
		pdet_input,				 //i-1, pattern detect
...
input 			ctrl_input;		 //i-1, Ctrl in signal from PHY
input			pdet_input;		 //i-1, pattern detect
...
begin
	...
	begin
		...
		data_in_dly		<=	data_in;
		data_in_dly1	<=	(first_data)?	8'hFB	:	
                                (last_data)? 8'hFD   :   data_in_dly;
		                  
		gmii_rx_dv_dly	<=	gmii_rx_dv;
		first_data		<=	(gmii_rx_dv & !gmii_rx_dv_dly);
		last_data		<=	(!gmii_rx_dv & gmii_rx_dv_dly);
		first_data_dly	<=	first_data;
		ctrl_in			<=	(first_data_dly)?	1'b0	:
								(!gmii_rx_dv_dly)?	1'b1	:	ctrl_in;
								
		pdet_in			<=	(first_data_dly)?	1'b0	:
								(!gmii_rx_dv_dly)?	1'b1	:	pdet_in;
		
		frame <= (ctrl_in & (data_in_dly1 == 8'hfb))? 1'b1 : ((ctrl_in & (data_in_dly1 == 8'hfd))? 1'b0 : frame);
		insert <= frame & !ctrl_in;
	
		sof <= ctrl_in & (data_in_dly1 == 8'hfb);
		eof <= ctrl_in & (data_in_dly1 == 8'hfd);
		eof_dly1	<= eof ;
		eof_dly2 <= eof_dly1;
		eof_seen	<=
		...
		if (fmac_speed == 2'd1)
			begin                  
				...
				x_byte_cnt <= (data_in_dly1 == 8'hfb & ctrl_in) ? 16'd1 :		   //Calculate byte count
			...
			begin
				...
				x_byte_cnt <= (data_in_dly1 == 8'hfb & ctrl_in) ? 16'd1 :	
...
begin
	frame_dly1 <= frame;
		//count relative to data_in_dly1
	counter2 <= 
		(!reset_) ? 8'h0 :
